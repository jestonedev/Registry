...
{
    public sealed class ResettleSubPremisesFromAssocDataModel: DataModel
    {
        private static ResettleSubPremisesFromAssocDataModel dataModel = null;
        private static string selectQuery = "SELECT * FROM resettle_sub_premises_from_assoc WHERE deleted = 0";
        private static string deleteQuery = "UPDATE resettle_sub_premises_from_assoc SET deleted = 1 WHERE id_assoc = ?";
        private static string insertQuery = @"INSERT INTO resettle_sub_premises_from_assoc (id_sub_premises, id_process) VALUES (?,?)";
        private static string updateQuery = @"UPDATE resettle_sub_premises_from_assoc SET id_sub_premises = ?, id_process = ? WHERE id_assoc = ?";
        private static string tableName = "resettle_sub_premises_from_assoc";

        private ResettleSubPremisesFromAssocDataModel(ToolStripProgressBar progressBar, int incrementor)
        ...
        public static ResettleSubPremisesFromAssocDataModel GetInstance()
        {
            return GetInstance(null, 0);
        }

        public static ResettleSubPremisesFromAssocDataModel GetInstance(ToolStripProgressBar progressBar, int incrementor)
        {
            if (dataModel == null)
                dataModel = new ResettleSubPremisesFromAssocDataModel(progressBar, incrementor);
            return dataModel;
        }
