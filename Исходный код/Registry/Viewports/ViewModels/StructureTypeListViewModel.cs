...
{
    internal sealed class StructureTypeListViewModel: SnapshotedViewModel
    {
        public StructureTypeListViewModel()
            : base(new Dictionary<string, ViewModelItem>
        {
            {"general", new ViewModelItem(EntityDataModel<StructureType>.GetInstance())}
        })
        {
        ...
        {
            SnapshotDataSource = new DataTable("snapshot_structure_types")
            {
        ...
        public override void LoadSnapshot()
        {
            ...
            foreach (var row in this["general"].BindingSource)
                SnapshotDataSource.Rows.Add(EntityConverter<StructureType>.ToArray((DataRowView)row));
        }
    }
}
