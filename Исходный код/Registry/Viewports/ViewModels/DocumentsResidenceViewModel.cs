...
{
    internal sealed class DocumentsResidenceViewModel: SnapshotedViewModel
    {
        public DocumentsResidenceViewModel() : base(new Dictionary<string, ViewModelItem>
        {    
            { "general", new ViewModelItem(EntityDataModel<DocumentResidence>.GetInstance()) }
        })
        {
        ...
        {
            SnapshotDataSource = new DataTable("snapshot_documents_residence")
            {
        ...
        public override void LoadSnapshot()
        {
            ...
            foreach (var row in this["general"].BindingSource)
                SnapshotDataSource.Rows.Add(EntityConverter<DocumentResidence>.ToArray((DataRowView)row));
        }
    }
}
